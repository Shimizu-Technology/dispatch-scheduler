...
function Metric({ icon, label, value, tone = 'blue', detail, onClick }: { icon: ReactNode; label: string; value?: number | string; tone?: Tone; detail?: string; onClick?: () => void }) {
  const content = <>
    <div className={`inline-flex rounded-xl border p-2 ${toneClasses(tone)}`}>{icon}</div>
    <div className="min-w-0">
      <div className="font-display tabular text-2xl font-black tracking-tight text-[#172033]">{value ?? 0}</div>
      <div className="text-[0.64rem] font-extrabold uppercase tracking-[0.16em] text-[#64748b]">{label}</div>
      {detail && <p className="mt-1 text-xs font-semibold leading-4 text-[#526071]">{detail}</p>}
    </div>
  </>

  const className = "group flex min-h-[6.5rem] items-start gap-3 rounded-2xl border border-[rgba(23,32,51,0.1)] bg-white/94 p-4 text-left shadow-[0_10px_26px_rgba(23,32,51,0.055)] transition hover:border-[#244393]/20 hover:shadow-[0_16px_34px_rgba(23,32,51,0.08)]"
...
export function DashboardMetrics({ dashboard, workOrders, teams, technicians, pmTasks, schedule, auditEvents, canEdit, working, onGoToSection, onSuggest }: DashboardMetricsProps) {
  ...
  return <section className="soft-reveal-delay space-y-4">
      ...
      <Card className="overflow-hidden border-[#244393]/14 bg-white text-[#172033] shadow-[0_16px_42px_rgba(23,32,51,0.075)]">
        <div className="grid gap-0 lg:grid-cols-[1fr_auto]">
          <div className="relative p-5">
            ...
            <p className="font-display text-[0.68rem] font-extrabold uppercase tracking-[0.24em] text-[#244393]">Today’s command decision</p>
            <h2 className="font-display mt-2 text-3xl font-black tracking-[-0.035em] text-[#172033]">{next.label}</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#526071]">{next.detail}</p>
          </div>
          <div className="flex min-w-[15rem] flex-col justify-center gap-2 border-t border-[rgba(23,32,51,0.1)] bg-[#f8faff] p-4 lg:border-l lg:border-t-0">
            {!schedule && canEdit && driverIssues === 0 && <button type="button" disabled={working} onClick={onSuggest} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#d84332] px-4 py-3 font-display text-sm font-extrabold text-white shadow-[0_14px_30px_rgba(216,67,50,0.24)] transition hover:-translate-y-0.5 hover:bg-[#bf3228] disabled:cursor-not-allowed disabled:opacity-60">
              <ClipboardList size={17} /> {working ? 'Working...' : 'Generate draft'}
            </button>}
            <button type="button" onClick={() => onGoToSection(next.target)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#244393]/15 bg-white px-4 py-3 font-display text-sm font-extrabold text-[#244393] transition hover:-translate-y-0.5 hover:bg-[#e8eefc]">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(20rem,0.85fr)]">
      <Card className="overflow-hidden">
        <PanelHeader eyebrow="Work pressure" title="Queue health" description="Open work, SLA/KPI pressure, blocked jobs, and follow-up counts for the selected date." />
