...
export function PaProjectsPanel({ workOrders, meta, onPage, onEdit, canEdit }: { workOrders: WorkOrder[]; meta: PaginationMeta | null; onPage: (page: number) => Promise<void>; onEdit: (workOrder: WorkOrder) => void; canEdit: boolean }) {
  ...
  return <Card className="overflow-hidden">
    <PanelHeader
      ...
      action={<div className="grid w-full grid-cols-3 gap-2 text-center text-[0.68rem] font-extrabold uppercase tracking-[0.08em] text-[#64748b] sm:w-auto sm:text-xs sm:tracking-[0.1em]">
        <span className="rounded-xl border border-[#244393]/15 bg-[#e8eefc] px-2 py-2 text-[#244393] sm:px-3">{totalCount} total</span>
        <span className="rounded-xl border border-amber-200 bg-amber-50 px-2 py-2 text-amber-900 sm:px-3">{waitingParts} parts</span>
        <span className="rounded-xl border border-indigo-200 bg-indigo-50 px-2 py-2 text-indigo-900 sm:px-3">{estimates} estimates</span>
      </div>}
    />
    <div className="space-y-3 p-3 sm:p-4">
      {paProjects.map((workOrder) => <article key={workOrder.id} className="rounded-2xl border border-[rgba(23,32,51,0.1)] bg-white p-3 shadow-[0_12px_28px_rgba(23,32,51,0.06)] sm:p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {canEdit && <button type="button" onClick={() => onEdit(workOrder)} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#244393]/15 bg-[#e8eefc] px-3 py-2 font-display text-xs font-extrabold uppercase tracking-[0.12em] text-[#244393] sm:w-auto"><ClipboardList size={14} /> Open work order</button>}
        </div>
