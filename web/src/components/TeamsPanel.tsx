...
export function TeamsPanel({ teams, onToggleAvailability }: { teams: Team[]; onToggleAvailability: (tech: Technician) => Promise<void> }) {
  return <Card>
        ...
        <div key={team.id} className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {team.technicians.map((tech) => (
              <button key={tech.id} onClick={() => onToggleAvailability(tech)} className={`rounded-full border px-3 py-1 text-xs font-bold ${tech.availability === 'unavailable' ? 'border-red-200 bg-red-50 text-red-700 line-through' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
