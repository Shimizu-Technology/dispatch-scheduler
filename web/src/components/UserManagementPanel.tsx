...
export function UserManagementPanel({ users, currentUserId, savingUserId, onRoleChange }: { users: ManagedUser[]; currentUserId: number | null; savingUserId: number | null; onRoleChange: (userId: number, role: ManagedUser['role']) => Promise<void> }) {
  return <Card className="overflow-hidden">
    ...
    <div className="space-y-3 p-4">
      {users.length === 0 && <p className="rounded-2xl border border-dashed border-[rgba(23,32,51,0.18)] bg-[#f8faff] p-5 text-sm font-semibold text-[#526071]">No users have signed in yet.</p>}
      {users.map((managedUser) => {
        const isCurrentUser = managedUser.id === currentUserId
        return <div key={managedUser.id} className="grid gap-4 rounded-2xl border border-[rgba(23,32,51,0.1)] bg-white/88 p-4 shadow-[0_10px_26px_rgba(23,32,51,0.05)] lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-display font-extrabold tracking-tight text-[#172033]">{managedUser.name}</h3>
            ...
            <p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-[#7b8798]">Last seen: {formatLastSeen(managedUser.last_seen_at)}</p>
          </div>
          <label className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#526071]">
            ...
            <select
              ...
              className="field-control mt-1 w-full min-w-48 rounded-xl px-3 py-2 text-sm font-bold normal-case tracking-normal text-[#172033] lg:w-auto"
            >
